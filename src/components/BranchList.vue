    ...
    <div class="flex gap-2.5 items-center h-10 shrink-0">
      <div class="relative flex-1 min-w-0 h-full">
        <input
          ...
          class="w-full h-full bg-app-solid dark:bg-app-glass/60 border border-app-border-light rounded-[var(--app-input-radius)] pl-4 pr-10 py-0 text-xs text-app-main placeholder-app-muted focus:outline-none focus:border-indigo-500/50 transition-all font-mono min-w-0 disabled:opacity-50"
        />
      ...
      <span v-if="branchesFetched && totalBranchesCount > 0" class="px-2.5 h-full bg-indigo-500/10 text-indigo-650 dark:text-indigo-400 text-[9px] font-black uppercase tracking-wider rounded-[var(--app-input-radius)] border border-indigo-500/20 whitespace-nowrap flex items-center shrink-0">
        ...
        <div class="flex items-center shrink-0 pr-3">
          <div class="w-5 h-5 rounded-md border-2 transition-all duration-200 flex items-center justify-center"
               :class="selectedBranches.includes(branch.name)
                 ...
                 : 'border-app-border dark:border-slate-700 bg-app-solid dark:bg-slate-950/80 group-hover:border-slate-400 dark:group-hover:border-slate-500'"
          >
            <svg
